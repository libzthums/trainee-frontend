...
export default function Main() {
  ...
  const [expireStatusOptions, setExpireStatusOptions] = useState([]);
  ...
  const filterData = useCallback(() => {
    ...
    return visibleData.filter((item) => {
      ...
      return (
        ...
        (!filters.statusQuery ||
          String(item.statusID) === String(filters.statusQuery)) &&
  ...
  }, [data, searchQuery, user, filters, user.divisionID]);

  useEffect(() => {
    fetchData();
  }, [fetchData, user.divisionID]);
  ...
  useEffect(() => {
    ...
    fetchExpireStatusOptions();
    fetchTypeList();
  }, [url]);

  const fetchExpireStatusOptions = async () => {
    try {
      const response = await axios.get(url + "service/expirechecklist");
      const options = [
      ...
      setExpireStatusOptions(options);
    } catch (error) {
      console.error("Error fetching expire status options:", error);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Badge
          pill
          bg={getStatusVariant(params.row?.statusID)}
          style={{ fontSize: "12px" }}>
          ...
          <Form>
            <Row className="px-3 mt-3">
              ...
              <Col md={6} className="mb-3">
                ...
                {tempFilters.statusQuery && (
                  <div
                    className={`mt-2 text-${getStatusVariant(
                      Number(tempFilters.statusQuery)
                    )}`}>
                    {
                      expireStatusOptions.find(
                        (opt) => opt.value === tempFilters.statusQuery
                      )?.label
                    }
                  </div>
                )}
